...
public OldInvoiceItem(GlobalApplicationDaemon g, Object[] i){
    this.application = g;
    item = i;
    this.db = application.getDb();
    VAT = DV.parseBool(application.getProps().getProp("VAT"), false);
}
public OldInvoiceItem(Object[] i, GlobalApplicationDaemon application, boolean vat){
...
public static ArrayList getItemList(DefaultTableModel t, GlobalApplicationDaemon application, boolean vat){
    ...
    for (int r = 0; r < t.getRowCount(); r++){
        al.add(new OldInvoiceItem(DV.getTableRow(t, r), application, vat));
    }
    al.trimToSize();
    return al;
}
...
public static ArrayList getItemList(DefaultTableModel t, GlobalApplicationDaemon g){
    ...
    for (int r = 0; r < t.getRowCount(); r++){
        al.add(new OldInvoiceItem(g, DV.getTableRow(t, r)));
    }
    al.trimToSize();
    return al;
}
...
public float getItemTotal(){
    ...
    if (isTax1() || isTax2()){
        ...
        OldInvoice inv = new OldInvoice(application, this.getInvoiceKey());
...
public float getTax1Total(){
    ...
    if (this.isTax1()){
        ...
        if (VAT) {
            ...
            return Tools.round(OldInvoice.extractGST(getUnitTotal(), taxRate));
        }else {
